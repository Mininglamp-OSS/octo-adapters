...
`vMAJOR.MINOR.PATCH` (e.g. `create-openclaw-octo/v1.1.0`):
...
> **Monorepo note:** this repo ships multiple packages, so tags are **package-scoped** — `<package>/vMAJOR.MINOR.PATCH` (e.g. `create-openclaw-octo/v1.0.1`). Version each package independently; the steps below are otherwise identical.
...
2. **Push the tag** on that exact commit:
   ```
   git tag -a create-openclaw-octo/v1.1.0 -m "Release create-openclaw-octo/v1.1.0" <sha>
   git push origin create-openclaw-octo/v1.1.0
   ```
